...
def test_high_risk_requires_review_and_is_advisory():
    payload = {
        ...
        "available_shelter_capacity": 20,
        "estimated_relocation_demand": 150,
    }
    result = client.post("/api/v1/ai/risk-assessment", json=payload)
...
def test_missing_features_reduce_confidence_and_can_be_unknown():
    body = client.post("/api/v1/ai/risk-assessment", json={"habitation_id": "HAB-002"}).json()
...
def test_invalid_population_is_rejected():
    result = client.post("/api/v1/ai/risk-assessment", json={"habitation_id": "HAB-003", "population": -1})
...
def test_capacity_shortage_and_unknown_capacity():
    payload = {
        ...
        "estimated_relocation_demand": 80,
        "shelter": {
            "total_capacity": 100,
    ...
    body = client.post("/api/v1/ai/relocation-priority", json=payload).json()
    assert body["result"]["capacity_gap"] == 50
    assert body["result"]["capacity_unknown"] is False
    unknown = client.post("/api/v1/ai/relocation-priority", json={"habitation_id": "HAB-005"}).json()
...
def test_report_is_pending_review_even_with_high_confidence():
    body = client.post(
        ...
        json={"report_id": "REP-001", "report_text": "The road is blocked by flood water", "nearby_hazard": True},
    ).json()
...
def test_model_status_and_validation():
    assert client.get("/api/v1/ai/model-status").json()["model_version"] == "rule-baseline-v1"
    result = client.post("/api/v1/ai/risk-assessment", json={"habitation_id": "H", "latitude": 91})
    assert result.status_code == 422


def test_backwards_compatible_ai_prefix():
    assert client.get("/ai/model-status").json()["model_version"] == "rule-baseline-v1"
    body = client.post("/ai/risk-assessment", json={"habitation_id": "HAB-ROOT"}).json()
    assert body["result"]["habitation_id"] == "HAB-ROOT"
